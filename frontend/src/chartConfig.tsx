...
export const CHARTS: ChartEntry[] = [
  ...
  },
  {
    ...
    desc: "純利益 = スループット(T) - 業務費用(OE)。緑棒が期間あたり完了タスク数(T)、赤線が投入作業時間(OE)。Tが増えつつOEが減るのが理想。OEが高いのにTが低い期間は非効率",
  ...
  },
  {
    key: "roi",
    title: "ROI (T / I)",
    desc: "投資収益率 = スループット(T) / 在庫(I)。期間あたり完了数÷WIP数。高いほど仕掛かり在庫が効率的にスループットへ変換されている。低い期間はWIPが多すぎる兆候",
    render: ({ tasks, unit }) => <RoiChart tasks={tasks} unit={unit} />,
  },
  {
    key: "cash-flow",
    title: "Cash Flow (TOC)",
    desc: "期間ごとの(完了数 - 新規着手数)を累積表示。右肩上がりなら在庫が減りスループットが勝っている健全な状態。右肩下がりなら着手過多で在庫が膨らんでいる",
  ...
  },
  {
    key: "file-table",
    title: "File Summary",
    desc: "ファイルごとのタスク数、TODO/DONE/CLOSE内訳、合計作業時間、平均・最大リードタイム。ヘッダクリックでソート",
    render: ({ tasks }) => <FileTable tasks={tasks} />,
  },
  {
    key: "task-table",
    title: "Task Details",
    desc: "全タスクの詳細一覧。タスク名、ファイル、ステータス、リードタイム、作業時間、セッション数。ヘッダクリックでソート",
